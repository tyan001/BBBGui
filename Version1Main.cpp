...
const long Version1Frame::ID_GAUGE1 = wxNewId();
...
Version1Frame::Version1Frame(wxWindow* parent,wxWindowID id):m_timer(this)
{
    ...
    Slider3 = new wxSlider(Panel2, ID_SLIDER3, 1, 1, 100, wxPoint(8,41), wxSize(80,27), 0, wxDefaultValidator, _T("ID_SLIDER3"));
    Gauge1 = new wxGauge(Panel2, ID_GAUGE1, 100, wxPoint(40,200), wxDefaultSize, 0, wxDefaultValidator, _T("ID_GAUGE1"));
    ...
    Connect(ID_SLIDER1,wxEVT_COMMAND_SLIDER_UPDATED,(wxObjectEventFunction)&Version1Frame::OnColumnSliderUpdated);
    Connect(ID_TEXTCTRL1,wxEVT_COMMAND_TEXT_UPDATED,(wxObjectEventFunction)&Version1Frame::OnColumnText);
    Connect(ID_SLIDER2,wxEVT_COMMAND_SLIDER_UPDATED,(wxObjectEventFunction)&Version1Frame::OnRowSliderUpdated);
    Connect(ID_TEXTCTRL2,wxEVT_COMMAND_TEXT_UPDATED,(wxObjectEventFunction)&Version1Frame::OnRowText);
    Connect(ID_BUTTON2,wxEVT_COMMAND_BUTTON_CLICKED,(wxObjectEventFunction)&Version1Frame::OnStopClick);
    Connect(ID_TEXTCTRL3,wxEVT_COMMAND_TEXT_UPDATED,(wxObjectEventFunction)&Version1Frame::OnPopulationText);
    Connect(ID_SLIDER3,wxEVT_COMMAND_SLIDER_UPDATED,(wxObjectEventFunction)&Version1Frame::OnPopulationSliderUpdated);
    Connect(ID_BUTTON1,wxEVT_COMMAND_BUTTON_CLICKED,(wxObjectEventFunction)&Version1Frame::OnRunClick);
    ...
    for (int k = 0; k < z_dim; k++)
    {
        for (int j = 0; j < y_dim; j++)
        {
            for (int i = 0; i < x_dim; i++ )
            {
                ...
                wxMilliSleep(5); // controls how quickly the houses spawn
...
void Version1Frame::GaugeProgress(){
    if(runClicked == true)
    {
        Gauge1->Pulse();
    }
...
void Version1Frame::OnRunClick(wxCommandEvent& event)
{
    runClicked = true;
    ...
    // Disable all sliders and text boxes when you press Run
    Slider1->Disable();
    Slider2->Disable();
    Slider3->Disable();
    TextCtrl1->Disable();
    ...
    // Hide Run button when clicked
    Button1->Disable();
    Button1->Hide();
}

void Version1Frame::OnColumnSliderUpdated(wxScrollEvent& event)
{
...
void Version1Frame::OnColumnText(wxCommandEvent& event)
{
...
void Version1Frame::OnRowSliderUpdated(wxScrollEvent& event)
{
    y_dim = Slider2->GetValue();
...
void Version1Frame::OnRowText(wxCommandEvent& event)
{
...
void Version1Frame::OnStopClick(wxCommandEvent& event)
{
    m_timer.Stop();
    done = true;
    ...
    // Enable all sliders and text boxes when user clicks Stop
    Slider1->Enable();
    Slider2->Enable();
    Slider3->Enable();
    TextCtrl1->Enable();
    ...
    // Show and Enable Run button when clicked
    Button1->Enable();
    Button1->Show();
}
...
void Version1Frame::OnPopulationText(wxCommandEvent& event)
{
...
void Version1Frame::OnPopulationSliderUpdated(wxScrollEvent& event)
{
